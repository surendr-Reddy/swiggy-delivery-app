...
console.log(props);
 const {name,avgRating,deliveryTime,cuisines,cloudinaryImageId}=props.resData.info
...
<div className="res-card" >
    ...
    <div className="resName"><h2>{name}</h2></div>
    <div className="resName"> <h3>{cuisines.join(",")}</h3></div>
    <div className="start-rating"><h4>Rating :{avgRating}</h4></div>
    <div className="delivery"><h4>Delivery Time:{deliveryTime}</h4></div>
</div>
...
const Body =()=>{
    return (
        ...
        <div className="restraurant-card">
           {restaurantCardsData.map((resCrad)=>{return <RestraurantCard key={resCrad.info.id} resData={resCrad} />})}
